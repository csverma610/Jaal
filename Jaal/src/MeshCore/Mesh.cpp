...
JMeshPtr JMesh::newObject()
{
    ...
    // Creats cycles: handle them later...
...
vector<string> JMesh :: getAttributeNames(int entity) const
{
    assert( entity >=0 && entity < 4);
    ...
    case 0:
        for( const JNodePtr &vertex: nodes) {
            if( vertex->isActive() ) {
                vnames = vertex->getAttributeNames();
...
JMesh::pruneNodes()
{
    ...
    vend = remove_if(nodes.begin(), nodes.end(), JEntityRemovedPred());
...
JMesh::pruneEdges()
{
    ...
    eend = remove_if(edges.begin(), edges.end(), JEntityRemovedPred());
...
JMesh::pruneFaces()
{
    ...
    fend = remove_if(faces.begin(), faces.end(), JEntityRemovedPred());
...
JMesh::pruneCells()
{
    ...
    cend = remove_if(cells.begin(), cells.end(), JEntityRemovedPred());
