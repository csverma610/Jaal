...
JMeshPtr read_mesh_file( const string &fname)
{
    ...
    while(!infile.eof() ) {
        infile >> str;
        if( str.compare("MeshVersionFormatted") == 0) infile >> str;
        if( str.compare("Dimension") == 0) infile >> str;
        if( str.compare("Vertices") == 0)  {
            infile >> numnodes;
            double x, y, z = 0.0;
            Point3D xyz;
        ...
        if( str.compare("Quadrilatrals")==0) {
            infile >> numfaces;
            connect.resize(4);
            for( size_t i = 0; i < numfaces; i++) {
