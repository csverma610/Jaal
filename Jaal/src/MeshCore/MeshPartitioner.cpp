...
int JMeshPartitioner ::search_edges_interface()
{
...
int JMeshPartitioner ::search_faces_interface()
{
    return 0;
}
///////////////////////////////////////////////////////////////////////////////
...
int JMeshPartitioner ::searchInterfaces()
{
    ...
    if( topDim == 2 ) search_edges_interface();
    if( topDim == 3 ) search_faces_interface();
...
/*
void JMeshPartitioner :: getPartition( JFaceSequence &ifaces)
{
    int topDim = mesh->getTopology()->getDimension();
    ...
    for( size_t i = 0; i < numfaces; i++) {
        ...
        if( face->isActive() ) {
            ...
            if( neighs.size() == 2 ) {
                neighs[0]->getAttribute("Partition", pid0);
                neighs[1]->getAttribute("Partition", pid1);
                if( pid0 != pid1 ) ifaces.push_back(face);
            }
        }
    }
}
