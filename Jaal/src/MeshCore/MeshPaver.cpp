...
void JMeshPaver :: init()
{
    ...
    elemType = mesh->getTopology()->isHomogeneous(topDim);
}
...
void JMeshPaver :: addNewLayer()
{
    ...
    for( int i = 0; i < numedges; i++) {
        ...
        if( elemType == JFace::QUADRILATERAL ) {
            newface = Quadrilateral::newObject( v3,v2,v1,v0);
        ...
        if( elemType == JFace::TRIANGLE ) {
            if( hybrid ) {
                ...
                newface = Triangle::newObject( v3,v2,v1);
                newfaces.push_back(newface);
                newface = Triangle::newObject( v3,v1,v0);
                newfaces.push_back(newface);
            }
        }
