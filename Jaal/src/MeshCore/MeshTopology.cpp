...
void JMeshTopology :: getOrphaned( JNodeSequence &orphans)
{
    ...
    rstl::set_difference(aset, bset, orphans);
}
...
void JMeshTopology :: getRim( const JNodePtr &vtx, JEdgeSequence &edges)
{
    ...
    rstl::copy(edgeSet, edges);
}

////////////////////////////////////////////////////////////////////////////////

bool JMeshTopology :: isDisk() const
{
    if( mesh == nullptr ) return 0;
    if( getDimension() == 3) return 0;

    // an edge is likely to have two neightbouring face...
//  std::map<JEdgePtr,boost::container::flat_set<JFacePtr>> edgeSet;
    std::map<JEdgePtr,std::set<JFacePtr>> edgeSet;

    // This procedure will work for the submesh also.
    size_t numfaces = mesh->getSize(2);
    for( size_t i = 0; i < numfaces; i++) {
        const JFacePtr &face = mesh->getFaceAt(i);
        if( face->isActive() ) {
            ...
            for( int j = 0; j < numedges; j++) {
                const JEdgePtr &edge = face->getEdgeAt(j);
                edgeSet[edge].insert(face);
            }
        }
    }

    JEdgeSequence bdedges;
    for( const auto &keyVal : edgeSet) {
        if( keyVal.second.size() == 1) bdedges.push_back(keyVal.first);
    }

    vector<JEdgeSequence> edgeloops;
    JEdgeTopology::getLoops( bdedges, edgeloops);
    if( edgeloops.size() == 1) return 1;
...
JMeshTopology::getNodesArray( vector<size_t> &nodearray, vector<int> &elmtopo)
{
    ...
    if( tDim == 3 ) {
        size_t numcells = mesh->getSize(3);
        topo = mesh->getTopology()->isHomogeneous( 3 );
    ...
    if( tDim == 2 ) {
        size_t numfaces = mesh->getSize(2);
        topo = mesh->getTopology()->isHomogeneous( 2 );
...
{
    if( mesh->hasTopologyChanged() ) {
        logger->setWarn("Mesh topology changed, boundary identification may be incorrect");
    }
    ...
    if( topDim == 3 ) {
        ...
        while (!cellQ.empty()) {
            ...
            if (cell->getVisitBit() == 0) {
                cell->setVisitBit(1);
                cout << "CellVisited " << cell->getVisitBit() << endl;
                cset.insert(cell);
        ...
        cout << "Debug: Press the Key " << cset.size() << "   #Cells " << numcells << endl;
        getchar();
        for (size_t icell = 0; icell < numcells; icell++) {
...
{
    logger->setInfo("Collecting boundary nodes");
    ...
    getBoundary(bedges);
    size_t numbedges = bedges.size();
    for( size_t i = 0; i < numbedges; i++) {
        for( int j = 0; j < 2; j++) {
            const JNodePtr &v = bedges[i]->getNodeAt(j);
            if( !v->getVisitBit() ) {
                bnodes.push_back(v);
                v->setVisitBit(1);
            }
...
void JMeshTopology :: getInternal( JNodeSequence &inodes)
{
    ...
    rstl::set_difference(allnodes,bdnodes, inodes);
}
...
void JMeshTopology :: getInternal( JEdgeSequence &iedges)
{
    ...
    rstl::set_difference(alledges, bdedges, iedges);
}
...
void JMeshTopology :: get_nonmanifold_edges( JEdgeSequence &bedges)
{
    if( getDimension() != 2 ) {
...
void JMeshTopology :: getInternal( JFaceSequence &ifaces)
{
    ...
    // Internal edges = alledges - boundary edges ...
    rstl::set_difference(allfaces, bdfaces, ifaces);
}
...
int
JMeshTopology ::isHomogeneous(int dim) const
{
    if( mesh == nullptr) return 0;
...
void JMeshTopology :: get_ordered_sequence(const JFacePtr &seedface, JFaceSequence &bfs, int order, size_t maxSize)
{
...
void JMeshTopology :: get_ordered_sequence(const JCellPtr &seedcell, JCellSequence &bfs, int order, size_t maxSize)
{
    mesh->buildRelations(2, 3);
