...
JMeshPtr AllTriMeshGenerator:: getSierpinski(int nlevels )
{
    ...
    JFacePtr baseTri = Triangle::newObject(v0,v1,v2);
    mesh->addObject(v0);
    mesh->addObject(v1);
    mesh->addObject(v2);
...
void sierpinski_refine_quad( const JFacePtr &face, JNodeSequence &newnodes, JFaceSequence &newfaces)
{
    ...
    newfaces.resize(8);
    newfaces[0] = Quadrilateral::newObject( qnodes[0], qnodes[1], qnodes[5], qnodes[4]);
    newfaces[1] = Quadrilateral::newObject( qnodes[1], qnodes[2], qnodes[6], qnodes[5]);
    newfaces[2] = Quadrilateral::newObject( qnodes[2], qnodes[3], qnodes[7], qnodes[6]);

    newfaces[3] = Quadrilateral::newObject( qnodes[4], qnodes[5], qnodes[9], qnodes[8]);
    newfaces[4] = Quadrilateral::newObject( qnodes[6], qnodes[7], qnodes[11], qnodes[10]);

    newfaces[5] = Quadrilateral::newObject( qnodes[8], qnodes[9], qnodes[13], qnodes[12]);
    newfaces[6] = Quadrilateral::newObject( qnodes[9], qnodes[10], qnodes[14], qnodes[13]);
    newfaces[7] = Quadrilateral::newObject( qnodes[10], qnodes[11], qnodes[15], qnodes[14]);
...
JMeshPtr AllQuadMeshGenerator:: getSierpinski( int nlevels )
{
    ...
    JFacePtr baseQuad = Quadrilateral::newObject(v0,v1,v2,v3);
    mesh->addObject(v0);
    mesh->addObject(v1);
    mesh->addObject(v2);
    mesh->addObject(v3);
...
void sierpinski_refine_tet( const JCellPtr &cell, JNodeSequence &newnodes, JCellSequence &newcells)
{
    ...
    newcells.resize(4);
    newcells[0] = Tetrahedron::newObject( qnodes[0], qnodes[4], qnodes[5], qnodes[6] );
    newcells[1] = Tetrahedron::newObject( qnodes[1], qnodes[9], qnodes[7], qnodes[4] );
    newcells[2] = Tetrahedron::newObject( qnodes[2], qnodes[5], qnodes[7], qnodes[8] );
    newcells[3] = Tetrahedron::newObject( qnodes[3], qnodes[6], qnodes[8], qnodes[9] );
}
