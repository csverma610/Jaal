...
int JMeshNonlinearOptimization::untangle()
{
    ...
    int topo = mesh->getTopology()->isHomogeneous(2);
    if( topo == JFace::QUADRILATERAL )
        jmesh = AllTriMeshGenerator::fromQuadMesh(jmesh,4);
...
int JMeshNonlinearOptimization::qualityOptimize2D()
{
    ...
    if( topo == 0 ) {
        ...
        JMeshPtr trimesh = AllTriMeshGenerator::fromPolyMesh(dummyMesh);
    ...
    if( topo == JFace::QUADRILATERAL ) {
        ...
        JMeshPtr dummyMesh = mesh->deepCopy();
        JMeshPtr trimesh = AllTriMeshGenerator::fromQuadMesh(dummyMesh, 4);
        execute(trimesh);
        size_t numNodes = mesh->getSize(0);
        size_t index = 0;
        for( size_t i = 0; i < numNodes; i++) {
...
{
    int topo = mesh->getTopology()->isHomogeneous(3);
...
//  #pragma omp parallel for
    ...
    int topo = mesh->getTopology()->isHomogeneous(meshDim);
...
int  JCyclicQuadMeshOptimizer :: atomicOp( const JNodePtr &vtx)
{
    ...
    double zlapCorr = 0.0;
    for( int i = 0; i < 4; i++) {
        JQuadrilateralPtr face = Quadrilateral::down_cast( faceneighs[i] );
        assert( face );
        const JNodePtr &v =  Quadrilateral::getDiagonalNode(face, vtx);
        xyz = v->getXYZCoords();
