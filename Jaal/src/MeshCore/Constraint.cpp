...
std::shared_ptr<Constraint> Constraint::shapeConstraintFactory(const std::string &constraintType, const std::vector<int> &idI, Scalar weight, const Matrix3X &positions) {
    ...
    if (constraintType.compare("EdgeStrain") == 0) {
        if (n != 2) {
            return c;
        }
        return std::make_shared<EdgeStrainConstraint>(idI, weight, positions);
    }
    if (constraintType.compare("TriangleStrain") == 0) {
        if (n != 3) {
            return c;
        }
        return std::make_shared<TriangleStrainConstraint>(idI, weight, positions);
    }
    if (constraintType.compare("TetrahedronStrain") == 0) {
        if (n != 4) {
            return c;
        }
        return std::make_shared<TetrahedronStrainConstraint>(idI, weight, positions);
    }
    ...
    if (constraintType.compare("Line") == 0) {
        if (n <  2) {
            return c;
        }
        return std::make_shared<LineConstraint>(idI, weight, positions);
    }
    if (constraintType.compare("Plane") == 0) {
        if (n <  3) {
            return c;
        }
        return std::make_shared<PlaneConstraint>(idI, weight, positions);
    }
    if (constraintType.compare("Circle") == 0) {
        if (n <  3) {
            return c;
        }
        return std::make_shared<CircleConstraint>(idI, weight, positions);
    }
    if (constraintType.compare("Sphere") == 0) {
        if (n <  4) {
            return c;
        }
        return std::make_shared<SphereConstraint>(idI, weight, positions);
    }
    if (constraintType.compare("Similarity") == 0) {
        if (n <  1) {
            return c;
        }
        return std::make_shared<SimilarityConstraint>(idI, weight, positions, true);
    }
    if (constraintType.compare("Rigid") == 0) {
        if (n <  1) {
            return c;
        }
        return std::make_shared<SimilarityConstraint>(idI, weight, positions, false);
    }
    if (constraintType.compare("Rectangle") == 0) {
        if (n != 4) {
            return c;
        }
        return std::make_shared<RectangleConstraint>(idI, weight, positions);
    }
    if (constraintType.compare("Parallelogram") == 0) {
        if (n != 4) {
            return c;
        }
        return std::make_shared<ParallelogramConstraint>(idI, weight, positions);
    }
    if (constraintType.compare("Laplacian") == 0) {
        if (n <  2) {
            return c;
        }
        return std::make_shared<UniformLaplacianConstraint>(idI, weight, positions, false);
    }
    if (constraintType.compare("LaplacianDisplacement") == 0) {
        if (n <  2) {
            return c;
        }
        return std::make_shared<UniformLaplacianConstraint>(idI, weight, positions, true);
    }
    if (constraintType.compare("Angle") == 0) {
        if (n !=  3) {
            return c;
        }
        return std::make_shared<AngleConstraint>(idI, weight, positions);
    }
    return c;
}
...
void TriangleStrainConstraint::setTarget(const Matrix3X &positions)
{
    Matrix32 edges, P;
    edges.col(0) = (positions.col(idI_[1]) - positions.col(idI_[0]));
    edges.col(1) = (positions.col(idI_[2]) - positions.col(idI_[0]));
...
void TetrahedronStrainConstraint::setTarget(const Matrix3X &positions)
{
    Matrix33 edges;
    for (int i = 0; i < 3; ++i) edges.col(i) = positions.col(idI_[i + 1]) - positions.col(idI_[0]);
...
void AreaConstraint::setTarget(const Matrix3X &positions)
{
    Matrix32 edges, P;
    edges.col(0) = (positions.col(idI_[1]) - positions.col(idI_[0]));
    edges.col(1) = (positions.col(idI_[2]) - positions.col(idI_[0]));
...
void VolumeConstraint::setTarget(const Matrix3X &positions)
{
    Matrix33 edges;
    for (int i = 0; i < 3; ++i) edges.col(i) = positions.col(idI_[i + 1]) - positions.col(idI_[0]);
    Matrix33 F = edges * rest_;
...
void BendingConstraint::setTarget(const Matrix3X &positions)
{
...
///////////////////////////////////////////////////////////////////////////////
void ClosenessConstraint::setTarget( const Matrix3X & /*positions*/)
{
    projPoints = rest_ * weight_;
}
///////////////////////////////////////////////////////////////////////////////
...
void LineConstraint::setTarget( const Matrix3X &positions)
{
    ...
    projPoints = (basis * input) * weight_;
}
...
void PlaneConstraint::setTarget(const Matrix3X &positions)
{
    for (int i = 0; i < static_cast<int>(idI_.size()); ++i) input.col(i) = positions.col(idI_[i]);
    Vector3 mean_vector = input.rowwise().mean();
    input.colwise() -= mean_vector;
...
void CircleConstraint::setTarget( const Matrix3X &positions)
{
    for (int i = 0; i < static_cast<int>(idI_.size()); ++i) input.col(i) = positions.col(idI_[i]);
    Vector3 mean_vector = input.rowwise().mean();
    input.colwise() -= mean_vector;
...
void SphereConstraint:: setTarget(const Matrix3X &positions)
{
    for (int i = 0; i < static_cast<int>(idI_.size()); ++i) input.col(i) = positions.col(idI_[i]);
    Vector3 mean_vector = input.rowwise().mean();
    input.colwise() -= mean_vector;
...
void SimilarityConstraint:: setTarget(const Matrix3X &positions)
{
    for (int i = 0; i < static_cast<int>(idI_.size()); ++i) input.col(i) = positions.col(idI_[i]);
    Vector3 mean_vector = input.rowwise().mean();
    input.colwise() -= mean_vector;
...
void RectangleConstraint::setTarget(const Matrix3X &positions)
{
    Matrix34 input;
    for (int i = 0; i < static_cast<int>(idI_.size()); ++i) input.col(i) = positions.col(idI_[i]);
    Vector3 mean_vector = input.rowwise().mean();
...
void ParallelogramConstraint::setTarget( const Matrix3X &positions)
{
    Matrix34 input;
    for (int i = 0; i < static_cast<int>(idI_.size()); ++i) input.col(i) = positions.col(idI_[i]);
    Vector3 mean_vector = input.rowwise().mean();
    ...
    projPoints = (basis * output) * weight_;
}
///////////////////////////////////////////////////////////////////////////////
...
void UniformLaplacianConstraint::setTarget(const Matrix3X & /*positions*/)
{
    projPoints = weighted_rest_;
}
...
void AngleConstraint:: setTarget(const Matrix3X &positions)
{
    /*
