...
int JMeshXMLImporter :: readFaces( const string &filename, const JMeshPtr &m)
{
    ...
    while( !infile.eof() ) {
        ...
        if( str == "<Triangles>" ) {
            ...
            assert( str == "</Count>" );
            nodes.resize(3);
            auto newtriangles = JTriangle::newObjects(ncount);
        ...
        if( str == "<Quads>" ) {
            ...
            assert( str == "</Count>" );
            nodes.resize(4);
            auto newquads = JQuadrilateral::newObjects(ncount);
            for( size_t i = 0; i < ncount; i++) {
                infile >> n1 >> n2 >> n3 >> n4;
                nodes[0] =  mesh->getNodeAt(n1);
                nodes[1] =  mesh->getNodeAt(n2);
        ...
        if( str == "<Polygons>" ) {
            ...
            infile >> str;
            assert( str == "</Count>" );
            auto newpolys = JPolygon::newObjects(ncount);
            for( size_t i = 0; i < ncount; i++) {
                infile >> nnodes ;
                nodes.resize(nnodes);
...
JMeshXMLExporter ::writeNodes(ofstream &ofile)
{
    ...
    vector<string> attribnames = mesh->getAttributeNames(0);
...
/*
    ...
    vector<string> attribnames = mesh->getAttributeNames(1);
    for( size_t i = 0; i < attribnames.size(); i++) {
        string name = attribnames[i];
...
JMeshXMLExporter ::writeFaces(ofstream &ofile)
{
    ...
    vector<string> attribnames = mesh->getAttributeNames(2);
    for( size_t i = 0; i < attribnames.size(); i++) {
        string name = attribnames[i];
...
JMeshXMLExporter ::writeCells(ofstream &ofile)
{
    ...
    vector<string> attribnames = mesh->getAttributeNames(3);
    for( size_t i = 0; i < attribnames.size(); i++) {
        string name = attribnames[i];
