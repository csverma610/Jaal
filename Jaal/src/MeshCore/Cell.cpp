...
JTetrahedronPtr JTetrahedron::getCanonical()
{
    ...
    auto tet = JTetrahedron::newObject();
    tet->setNodes(nodes);
    return tet;
}
...
int JTetrahedron ::  getOrientation( const JEdgePtr &edge) const
{
    ...
    for( int i = 0; i < NumEdges; i++) {
        if( getEdgeAt(i) == edge ) {
            getEdgeTopology(i, i0, i1);
            auto ve0 = getNodeAt(i0);
            auto ve1 = getNodeAt(i1);
            auto va0 = edge->getNodeAt(0);
            auto va1 = edge->getNodeAt(1);
...
int JTetrahedron :: getEdges( JEdgeSequence &edges) const
{
...
int JTetrahedron :: getFaces( JFaceSequence &faces)  const
{
...
{
    assert(cell->getSize(0) == 4);
    const auto &pa = cell->getNodeAt(0)->getXYZCoords();
    const auto &pb = cell->getNodeAt(1)->getXYZCoords();
    const auto &pc = cell->getNodeAt(2)->getXYZCoords();
    const auto &pd = cell->getNodeAt(3)->getXYZCoords();
...
JHexahedronPtr JHexahedron::getCanonical( double len )
{
    ...
    auto hex = JHexahedron::newObject();
    hex->setNodes(nodes);
    return hex;
}
////////////////////////////////////////////////////////////////////////////
...
JFacePtr JHexahedron :: getOppositeFace( const JFacePtr &f) const
{
    ...
    if(v[0] == 0 && v[1] ==  3 && v[2] == 4 && v[3] == 7 )  {
        ...
        vhash   = rstl::min_element(facenodes );
...
int JHexahedron :: getNeighbors( const JEdgePtr &edge, JFaceSequence &efaces)
{
    ...
    auto v0 = edge->getNodeAt(0);
    auto v1 = edge->getNodeAt(1);
...
int JCell :: get_shared_entities( const JCellPtr &cell1, const JCellPtr &cell2, JEdgeSequence &comm_edges)
{
    ...
    JEdgeSequence cell1edges, cell2edges;

    cell1->getEdges(cell1edges);
    cell2->getEdges(cell2edges);

    rstl::sort( cell1edges );
    rstl::sort( cell2edges );

    rstl::set_intersection(cell1edges, cell2edges, comm_edges);
...
int JCell :: get_shared_entities( const JCellPtr &cell1, const JCellPtr &cell2, JFaceSequence &comm_faces)
{
    ...
    JFaceSequence cell1faces, cell2faces;

    cell1->getFaces(cell1faces);
    cell2->getFaces(cell2faces);

    rstl::sort( cell1faces );
    rstl::sort( cell2faces );

    rstl::set_intersection( cell1faces, cell2faces, comm_faces);
...
int JHexahedron :: rotate_common_face( const JMeshPtr &mesh, const JHexahedronPtr &hex1, const JHexahedronPtr &hex2, int dir)
{
    ...
    const auto &v0 = comm_faces[0]->getNodeAt(0);
    const auto &v1 = comm_faces[0]->getNodeAt(1);
    const auto &v2 = comm_faces[0]->getNodeAt(2);
    const auto &v3 = comm_faces[0]->getNodeAt(3);

    const auto &v4 = hex1->getDiagonalNode(v2);
    const auto &v5 = hex1->getDiagonalNode(v3);
    const auto &v6 = hex1->getDiagonalNode(v0);
    const auto &v7 = hex1->getDiagonalNode(v1);

    const auto &v8  = hex2->getDiagonalNode(v2);
    const auto &v9  = hex2->getDiagonalNode(v3);
    const auto &v10 = hex2->getDiagonalNode(v0);
    const auto &v11 = hex2->getDiagonalNode(v1);
...
int JHexahedron :: getEdges( JEdgeSequence &edges) const
{
    edges.resize(NumEdges);
...
////////////////////////////////////////////////////////////////////////////////////
int JHexahedron :: getFaces( JFaceSequence &faces) const
{
    faces.resize(6);
    ...
    for( int i = 0; i < 6; i++) {
        ...
        auto v0 = getNodeAt(p0);
        auto v1 = getNodeAt(p1);
        auto v2 = getNodeAt(p2);
        auto v3 = getNodeAt(p3);
...
JTriangularPrismPtr JTriangularPrism::getCanonical()
{
    ...
    auto tp = JTriangularPrism::newObject();
    tp->setNodes(vnodes);
    return tp;
}
...
int JTriangularPrism :: getEdges( JEdgeSequence &edges) const
{
    edges.resize( NumEdges);
    JNodePtr v0, v1;
...
int JTriangularPrism :: getFaces( JFaceSequence &faces)  const
{
    faces.resize(NumFaces);
