...
bool JSwapTriEdge::isSwappable(const JEdgePtr &edge, bool check567)
{
    ...
    nodes[2] = Triangle::getOppositeNode(faceNeighs[0], nodes[0], nodes[1] );
    nodes[3] = Triangle::getOppositeNode(faceNeighs[1], nodes[0], nodes[1] );
...
int JSwapTriEdge::commit()
{
    ...
    if( oldt1->getNodeAt(pos+1) == ov1) {
        newt1 = Triangle::newObject( v1, ov1, ov2);
        newt2 = Triangle::newObject( v2, ov2, ov1);
    }

    if( oldt1->getNodeAt(pos+2) == ov1) {
        newt1 = Triangle::newObject( v1, ov2, ov1);
        newt2 = Triangle::newObject( v2, ov1, ov2);
    }
...
bool JSwapTriEdge::isSharp() const
{
    ...
    Vec3D A, B;
    A = TriGeometry::getNormal(ov1, v1, v2);
    B = TriGeometry::getNormal(ov2, v2, v1);
