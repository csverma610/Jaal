...
bool JTriDecimator :: isCollapsable(const JEdgePtr &edge, bool check567 )
{
    ...
    if( check567 ) {
        JNodePtr vo1 =  Triangle::getOppositeNode(faceneighs[0], v0, v1);
        if( vo1->getNumRelations(2) <= 6) return 0;

        JNodePtr vo2 =  Triangle::getOppositeNode(faceneighs[1], v0, v1);
...
int JTriDecimator :: remove_degree3_node( const JNodePtr &vertex)
{
    ...
    newFaces.resize(1);
    newFaces[0] = Triangle::newObject( nodeneighs );
...
int JTriDecimator :: remove_degree4_node( const JNodePtr &vertex)
{
    ...
    coveredges.resize(4);
    for( int i = 0; i < 4; i++) {
        coveredges[i] =  Triangle::getOppositeEdge(faceneighs[i], vertex);
    }
    ...
    if( v1->getNumRelations(2) >= 6 && v3->getNumRelations(2) >= 6)  {
        newFaces.resize(2);
        newFaces[0] = Triangle::newObject( v0, v1, v2);
        newFaces[1] = Triangle::newObject( v0, v2, v3);
        retain[0]   = v0;
        retain[1]   = v2;
    }
    ...
    if( newFaces.empty() ) {
        if( v0->getNumRelations(2) >= 6 && v2->getNumRelations(2) >= 6)  {
            newFaces.resize(2);
            newFaces[0] = Triangle::newObject( v0, v1, v3);
            newFaces[1] = Triangle::newObject( v1, v2, v3);
            retain[0]   = v1;
            retain[1]   = v3;
        }
    }
...
//
    ...
    // seven neighbours.
    newFaces.resize(7);
    newFaces[0]  = Triangle::newObject( vnew, vertex, newneighs[0]);
    for( int i = 0; i < 5; i++)
        newFaces[i+1]  = Triangle::newObject( vnew,  newneighs[i], newneighs[i+1]);
    newFaces[6]  = Triangle::newObject( vertex, vnew, newneighs[5]);
    mesh->addObjects( newFaces);
