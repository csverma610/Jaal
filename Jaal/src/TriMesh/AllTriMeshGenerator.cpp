...
////////////////////////////////////////////////////////////////////////////////
JMeshPtr AllTriMeshGenerator :: fromQuad2Tri( int *dim,  double *len, double *org, bool texCoord)
{
    ...
    for (int j = 0; j < ny - 1; j++) {
        for (int i = 0; i < nx - 1; i++) {
            ...
            JFacePtr t1 =  Triangle::newObject( v0, v1, v2);
            JFacePtr t2 =  Triangle::newObject( v0, v2, v3);
            faces[index++] = t1;
            faces[index++] = t2;
        }
    }
...
JMeshPtr AllTriMeshGenerator :: fromQuad4Tri( int *dim,  double *len, double *org, bool texCoord)
{
    if( dim == nullptr ) return nullptr;
    ...
    for (int j = 0; j < ny - 1; j++) {
        for (int i = 0; i < nx - 1; i++) {
            ...
            mesh->addObject(v0);
            faces[index++] = Triangle::newObject(v0, v1, v2);
            faces[index++] = Triangle::newObject(v0, v2, v3);
            faces[index++] = Triangle::newObject(v0, v3, v4);
            faces[index++] = Triangle::newObject(v0, v4, v1);
        }
    }
    mesh->addObjects(faces);
...
JMeshPtr AllTriMeshGenerator :: fromQuadMesh( const JMeshPtr &quadmesh, JNodeSequence &steiner, int trisPerQuad)
{
    JMeshPtr trimesh;
    ...
    if( trisPerQuad  == 4 ) {
        ...
        for( size_t i = 0; i < numfaces; i++) {
            ...
            if( f->isActive()  && f->getSize(0) == 4) {
                int pos = JFaceGeometry::concaveAt(f);
                if( pos >= 0) {
                ...
                for( int j = 0; j < 4; j++) {
                    ...
                    tface  = Triangle::newObject(nodes);
        ...
        for( size_t i = 0; i < numfaces; i++) {
            ...
            if( f->isActive() && f->getSize(0) == 4 ) {
                ...
                if( pos >= 0) {
                    ...
                    nodes[0] = f->getNodeAt(0);
                    nodes[1] = f->getNodeAt(1);
                    nodes[2] = f->getNodeAt(2);
                    tface    = Triangle::newObject( nodes );
                    trimesh->addObject( tface );

                    nodes[0] = f->getNodeAt(0);
                    nodes[1] = f->getNodeAt(2);
                    nodes[2] = f->getNodeAt(3);
                    tface    = Triangle::newObject( nodes );
                    trimesh->addObject( tface );
                }
            }
...
///////////////////////////////////////////////////////////////////////////////
JMeshPtr AllTriMeshGenerator :: fromQuadMesh( const JMeshPtr &quadmesh, int trisPerQuad)
{
    JNodeSequence steiner;
    return AllTriMeshGenerator :: fromQuadMesh(quadmesh, steiner, trisPerQuad);
}

///////////////////////////////////////////////////////////////////////////////
JMeshPtr AllTriMeshGenerator :: fromPolyMesh( const JMeshPtr &polymesh, JNodeSequence &steiner)
{
    JMeshPtr trimesh;
...
///////////////////////////////////////////////////////////////////////////////
JMeshPtr AllTriMeshGenerator :: fromPolyMesh( const JMeshPtr &polymesh)
{
    JNodeSequence steiner;
    return AllTriMeshGenerator :: fromPolyMesh(polymesh, steiner);
}
///////////////////////////////////////////////////////////////////////////////
