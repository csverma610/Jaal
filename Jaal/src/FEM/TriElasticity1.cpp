                            ...
                            vector<Point2D> &cPoints)
{
    ...
    Polygon_2 P;
    P.push_back (Point_2 (aPoints[0][0], aPoints[0][1]));
    P.push_back (Point_2 (aPoints[1][0], aPoints[1][1]));
    P.push_back (Point_2 (aPoints[2][0], aPoints[2][1]));

    Polygon_2 Q;
    Q.push_back (Point_2 (bPoints[0][0], bPoints[0][1]));
    Q.push_back (Point_2 (bPoints[1][0], bPoints[1][1]));
    Q.push_back (Point_2 (bPoints[2][0], bPoints[2][1]));
...
void TriElasticity :: setElementType(int e)
{
    femshape.setElementType(e);
...
void TriElasticity :: BMatrix(const double *uv, const Matrix2d &invJ, MatrixXd &B)
{
...
void TriElasticity :: BMatrix(const Face *face, const double *uv, MatrixXd &B)
{
    // Calculate the "B" matrix at any point within the trinagle. We pass the inverse
    // of the jacobian of the face, where we need to find the matrix "B". Notice that
    // we are pssing Jacobian of the face and not the face object, because the Jacobian
...
void TriElasticity :: initMesh()
{
    ...
    if( mesh->getTopology()->isHomogeneous(2) != JFace::TRIANGLE) {
                                  ...
                                  map<int,double> &fixMap, vector<int> &freeDof)
{
    ...
    freeDof.clear();
    set_difference(fullDof.begin(), fullDof.end(), fixDof.begin(), fixDof.end(),
                   back_inserter(freeDof));
