...
bool JQuadChord :: isCyclic() const
{
    ...
    if( !chordEdges[0]->isBoundary() ) return 1;
    return 0;
}
...
void JQuadChord :: expand(const JEdgePtr &thisEdge, const JFacePtr &thisFace)
{
    ...
    JQuadrilateralPtr quad = Quadrilateral::down_cast(thisFace);
    assert( quad );

    // Get the opposite edge in the given face ...
    JEdgePtr nextEdge = Quadrilateral::getOppositeEdge( thisFace, thisEdge );
    assert( nextEdge );
