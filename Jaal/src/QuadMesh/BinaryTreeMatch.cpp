...
int JBinaryTreeMatch::match_boundary_triangle(JFacePtr btri)
{
    ...
    btri->setStatus( MeshEntity::REMOVE );

    Point3D p3d;
    Vertex::getMidPoint(bv0, bv1, p3d);

    JNodePtr bound = Vertex::newObject();
                                   ...
                                   TreeNode *child2)
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        JEdgePtr edge = Simplex::getEdgeOf(v0,v1);
        edge->getRelations(neighs);
    ...
    connect[1] = ev0;
    connect[2] = ev1;
    qface = Triangle::newObject( connect );
    qface->setID(maxfaceID);
    JNodePtr dc1 = DualGraph::newObject(qface );
    dc1->setID(maxfaceID);
    ...
    qface = Triangle::newObject( connect );
    qface->setID(maxfaceID);
    JNodePtr dc2 = DualGraph::newObject(qface);
    dc2->setID(maxfaceID);
    trimesh->addObject(qface);
    steinerFaces.push_back(qface);
    dnode = nullptr;
...
void JBinaryTreeMatch::maximum_tree_matching()
{
    ...
    trimesh->getTopology()->search_boundary();
    ...
    DualGraph dgrapher;
    JMeshPtr dgraph = dgrapher.getGraph(trimesh);
...
{
    JMeshTopology *topology = inmesh->getTopology();
    if (topology->isHomogeneous(2) != JFace::TRIANGLE) {
    ...
    JMeshPtr quadmesh = AllQuadMeshGenerator::collapse_matched_triangles(trimesh, facematching);
