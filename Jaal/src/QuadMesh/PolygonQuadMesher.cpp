...
JMeshPtr JPolygonQuadMesher :: getTemplate( const vector<int> &segments)
{
    //************************************************************************
    ...
    system("quadpatch patch.dat patch.off");
    ...
    JMeshIO::saveAs(patch, "patch.off");
...
JMeshPtr JPolygonQuadMesher :: getPatch( const JNodeSequence &srcnodes, const vector<int> &segments)
{
...
JMeshPtr JPolygonQuadMesher :: getLoftPatch( const JNodeSequence &side1, const JNodeSequence &side2,
                                             const JNodeSequence &side3, const JNodeSequence &side4)
{
...
JMeshPtr JPolygonQuadMesher :: getAdaptivePatch( const JNodeSequence &srcNodes, const vector<int> &segments, double factor)
{
...
void  JPolygonQuadMesher :: UV2XYZMap(const JMeshPtr &meshtemplate,
                                      const JNodeSequence &srcnodes, const vector<int> &segments)
{
...
JMeshPtr JPolygonQuadMesher :: getTemplate( const JMeshPtr &m, int nside)
{
    mesh = m;
    ...
    /*
        ...
        if( nCount) {
            JMeshPtr tmesh = AllTriMeshGenerator::fromQuadMesh(meshtemplate,2);
...
int JPolygonQuadMesher :: stitchBoundary( const JMeshPtr &meshtemplate, const JNodeSequence &chainnodes)
{
    ...
    for( size_t i = 0; i < numfaces; i++) {
        ...
        JFacePtr newface = Quadrilateral::newObject( newConnect);
        newface->setAttribute("Partition", id);
    ...
    // Boundary nodes of the mesh template can be removed ...
    for( size_t i = 0; i < numbound; i++) {
...
int JPolygonQuadMesher :: nsidedQuads( const JFacePtr &face)
{
...
int JPolygonQuadMesher :: getConvexPatches()
{
...
int JPolygonQuadMesher :: genEdgeNodes()
{
...
/////////////////////////////////////////////////////////////////////////////
int JPolygonQuadMesher :: make_even_segments(JEdgeSequence &edgeloop, const JEdgePtr &edge)
{
...
QDefectivePatchPtr JPolygonQuadMesher :: getDefectivePatch( const JNodePtr &vtx)
{
    /*
        if( geodesic_path ) {
...
QDefectivePatchPtr JPolygonQuadMesher :: getDefectivePatch( const JFacePtr &face)
{
    /*
        if( geodesic_path ) {
            mesh->buildRelations(0,0);
...
QDefectivePatchPtr JPolygonQuadMesher :: getDefectivePatch( const JNodeSequence &nodepath)
{
    QDefectivePatchPtr  patch = boost::shared_ptr<QDefectivePatch>(new QDefectivePatch);
...
/////////////////////////////////////////////////////////////////////////////
void JPolygonQuadMesher :: reorderSingularities()
{
...
QDefectivePatchPtr JPolygonQuadMesher :: getAnyDefectivePatch()
{
...
int JPolygonQuadMesher :: remesh( const QDefectivePatchPtr  &patch)
{
    if( patch == nullptr)  return 1;
    if( !patch->isValid()) return 1;

    JFaceSequence faces = patch->getFaces();

    JNodeSequence oldInternalNodes;
    JEdgeSequence oldInternalEdges;
    mesh->getTopology()->getSubmeshInternal(faces, oldInternalEdges, oldInternalNodes);
    ...
    meshtemplate->getTopology()->getInternal(newInternalEdges, newInternalNodes);

    mesh->addObjects(newInternalNodes);
    mesh->addObjects(newInternalEdges);

    faces = meshtemplate->getFaces();
...
/////////////////////////////////////////////////////////////////////////////
void  JPolygonQuadMesher :: straighten( const JEdgePtr &edge)
{
    JNodeSequence nodes;
...
int JPolygonQuadMesher :: getBoundarySingularGraph( JEdgeSequence &edgeloop)
{
...
/////////////////////////////////////////////////////////////////////////////
JMeshPtr JPolygonQuadMesher :: getBoundarySingularGraph()
{
    if( mesh == nullptr) return nullptr;
...
int JPolygonQuadMesher :: segmentBoundary()
{
...
void JPolygonQuadMesher :: getVoxelMesh()
{
    size_t numnodes = mesh->getSize(0);
...
JNodeSequence JPolygonQuadMesher :: getInteriorSingularNodes()
{
    JNodeSequence nodes;
...
JMeshPtr JPolygonQuadMesher :: getQuadMesh()
{
    if( mesh == nullptr) return nullptr;
...
////////////////////////////////////////////////////////////////////////////////////
void JPolygonQuadMesher :: setMesh( const JMeshPtr &m)
{
    mesh.reset();
    if( m->getSize(2) == 0) {
...
void JPolygonQuadMesher :: pillowPatch( const JMeshPtr &patch, const vector<int> &segments, int minQuads)
{
...
int JPolygonQuadMesher :: applyLIM( const JMeshPtr &quadmesh)
{
...
void JPolygonQuadMesher :: updateDistanceMap( const JNodePtr &vi)
{
...
QDefectivePatchPtr JPolygonQuadMesher :: getGlobalPatch()
{
    if( distMap.empty() ) {
        JNodeSequence nodes = getInteriorSingularNodes();
...
int JPolygonQuadMesher :: splitPolygon( const JFacePtr &oldface)
{
    ...
    {
    case 3:
        face2 = Triangle::newObject( nodes);
        break;
    case 4:
        face2 = Quadrilateral::newObject( nodes);
        break;
    default:
        face2 = Polygon::newObject( nodes);
        break;
    }
...
JMeshPtr JPolygonQuadMesher :: getQuadTemplate( const JFacePtr &face, double edgelen)
{
    /*
...
QDefectivePatchPtr JPolygonQuadMesher :: buildCircularPatch(const JNodePtr &vtx)
{
    /*
        QDefectivePatchPtr  patch = boost::shared_ptr<QDefectivePatch>(new QDefectivePatch);
