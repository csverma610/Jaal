...
JTrianglePtr PenroseTiling :: getCanonical( int type , double l)
{
    ...
    JTrianglePtr tri = Triangle::newObject(v0,v1,v2);
    return tri;
}
...
void PenroseTiling :: refine36(JFacePtr intri )
{
    ...
    JFacePtr tri1 = Triangle::newObject(vC, vP, vB);
    tri1->setAttribute("Tile", 0);

    JFacePtr tri2 = Triangle::newObject(vP, vC, vA);
...
void PenroseTiling :: refine108( JFacePtr intri)
{
    ...
    JFacePtr tri1 = Triangle::newObject(vR, vQ, vA);
    tri1->setAttribute("Tile", 0);

    JFacePtr tri2 = Triangle::newObject(vQ, vR, vB);
    tri2->setAttribute("Tile", 1);

    JFacePtr tri3 = Triangle::newObject(vR, vC, vA);
...
void PenroseTiling :: refine( JFacePtr face )
{
    ...
    JTrianglePtr tri  = Triangle::down_cast(face);
...
void PenroseTiling :: execute()
{
    ...
    for( int i = 0; i < 10; i++) {
        ...
        if( i%2 == 0)
            t = Triangle::newObject(v0,v1,v2);
        else
            t = Triangle::newObject(v0,v2,v1);
