...
int  JPolyCubes :: reorient( const JFacePtr &face)
{
    ...
    if( xn > 0.8 && xn > yn && xn > zn) {
        if( normal[0] >  0.0 ) {
            side = Hexahedron::RIGHT_SIDE;
            face->setAttribute( "CubeSide", side);
        }
        if( normal[0] <  0.0 ) {
            side = Hexahedron::LEFT_SIDE;
            face->setAttribute( "CubeSide", side);
        }
    }
    ...
    if( yn > 0.8 && yn > xn && yn > zn ) {
        if( normal[1] >  0.0 ) {
            side = Hexahedron::TOP_SIDE;
            face->setAttribute( "CubeSide", side);
        }
        if( normal[1] <  0.0 ) {
            side = Hexahedron::BOTTOM_SIDE;
            face->setAttribute( "CubeSide", side);
        }
    }
    ...
    if( zn > 0.8 && zn > xn && zn > yn ) {
        if( normal[2] >  0.0 ) {
            side = Hexahedron::FRONT_SIDE;
            face->setAttribute( "CubeSide", side);
        }
        if( normal[2] <  0.0 ) {
            side = Hexahedron::BACK_SIDE;
            face->setAttribute( "CubeSide", side);
        }
    }
...
JMeshPtr JPolyCubes :: getPolyHexMesh()
{
    ...
    for( size_t i = 0; i < numCells; i++) {
        ...
        if( cell->isActive() ) {
            ...
            JHexahedronPtr h = Hexahedron::newObject();
            h->setNodes( connect);
