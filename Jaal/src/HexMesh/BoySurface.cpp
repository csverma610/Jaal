...
void JBoySurface:: getQuadMesh(int nx, int ny)
{
    for( int j = 0; j < ny-1; j++) {
        for( int i = 0; i < nx-1;  i++) {
            ...
            JFacePtr face = Quadrilateral::newObject( v0, v1, v2, v3 );
...
JMeshPtr JBoySurface :: getMin1()
{
    ...
    // [agfi]
    JFacePtr q1 = Quadrilateral::newObject( nodes[0], nodes[6], nodes[5], nodes[8] );

    // [cieh]
    JFacePtr q2 = Quadrilateral::newObject( nodes[2], nodes[8], nodes[4], nodes[7] );

    // [bhdg]
    JFacePtr q3 = Quadrilateral::newObject( nodes[1], nodes[7], nodes[3], nodes[6] );

    // [abhe]
    JFacePtr q4 = Quadrilateral::newObject( nodes[0], nodes[1], nodes[7], nodes[4] );

    // [bfic]
    JFacePtr q5 = Quadrilateral::newObject( nodes[1], nodes[5], nodes[8], nodes[2] );

    // [acdg]
    JFacePtr q6 = Quadrilateral::newObject( nodes[0], nodes[2], nodes[3], nodes[6] );

    // [aie]
    JFacePtr t1 = Triangle::newObject( nodes[0], nodes[8], nodes[4] );

    // [bgf]
    JFacePtr t2 = Triangle::newObject( nodes[1], nodes[6], nodes[5] );

    // [abc]
    JFacePtr t3 = Triangle::newObject( nodes[0], nodes[1], nodes[2] );

    // [chd]
    JFacePtr t4 = Triangle::newObject( nodes[2], nodes[7], nodes[3] );
