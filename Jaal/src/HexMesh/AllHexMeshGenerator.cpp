...
JMeshPtr AllHexMeshGenerator::getStructuredMesh(int *node_dim, double *length, double *origin)
{
    ...
    JCellSequence cells = Hexahedron::newObjects((nx-1) * (ny-1) * (nz-1));
...
int AllHexMeshGenerator :: stackQuadMesh( const JMeshPtr &mesh1, const JMeshPtr &mesh2, JCellSequence &newcells)
{
    ...
    for( size_t j = 0; j < numFaces; j++) {
        ...
        JCellPtr hex = Hexahedron::newObject(f0, f1);
...
int AllHexMeshGenerator :: removeDoublet( const JMeshPtr &mesh, const JNodePtr &vdoublet)
{
    ...
    JHexahedronPtr hex1 = Hexahedron::down_cast( cellneighs[0] );
    JHexahedronPtr hex2 = Hexahedron::down_cast( cellneighs[1] );

    JFaceSequence faceneighs;
    Hexahedron::get_shared_entities( hex1, hex2, faceneighs);
    ...
    JCellPtr mergedHex = Hexahedron::newObject( newfaces[0], newfaces[1], newfaces[2],
...
{
    if( tetmesh->getTopology()->isHomogeneous(3) != JCell::TETRAHEDRON) {
...
//                  steiner->setBoundaryMark( max(1,face->getBoundaryMark() ));
    ...
    for( size_t i = 0; i < numcells; i++) {
        JCellPtr cell = tetmesh->getCellAt(i);
        if( cell->isActive() ) {
            JTetrahedronPtr tet = Tetrahedron::down_cast(cell);
            tet->getHexCells(hex);
...
{
    int homog = m->getTopology()->isHomogeneous(2);
