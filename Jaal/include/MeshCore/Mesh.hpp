...
class JMeshGeometry;
typedef std::shared_ptr<JMeshGeometry> JMeshGeometryPtr;

class JMeshTopology;
typedef std::shared_ptr<JMeshTopology>  JMeshTopologyPtr;
...
typedef std::shared_ptr<JMeshFilter>  JMeshFilterPtr;
...
typedef std::shared_ptr<JMeshRelationManager> JMeshRelationManagerPtr;
...
public:
    ...
    */
    ...
    int  getAttributeType(const std::string &s, int entity) const;

    //  What attributes are attached to the given entity..
    std::vector<std::string> getAttributeNames(int entity) const;
    ...
    bool hasAttribute( const std::string &name, int e) const;

    size_t getNumAttributes( const std::string &name, int e) const;

    // Assign the same Attribute value to each edge in the given sequence ...
    template<class T>
    void setAttribute( JEdgeSequence &eseq, const std::string &name, const T &val)
    {
        for(JEdgePtr edge: eseq) edge->setAttribute(name, val);
    }

    template<class T>
    void getEntities( const std::string &name, const T &val, JNodeSequence &vseq)
    {
        vseq.clear();
        T thisval;
    ...
    template<class T>
    void getEntities( const std::string &name, const T &val, JEdgeSequence &eseq)
    {
        eseq.clear();
        T thisval;
    ...
    void getEntities( const std::string &name, JEdgeSequence &eseq)
    {
        eseq.clear();
        for(JEdgePtr edge : edges) {
    ...
    template<class T>
    void getEntities( const std::string &name, const T &lowVal, const T &highVal,
                      JEdgeSequence &eseq)
    {
        eseq.clear();
        T thisval;
    ...
    void setGeometry( const JMeshGeometryPtr &geom)
    {
        geometry = geom;
    }

    JMeshGeometryPtr getGeometry() const
    {
        return geometry;
    }

    void setTopology( const JMeshTopologyPtr &topo)
    {
        topology = topo;
    }

    JMeshTopologyPtr getTopology() const
    {
        return topology;
    }
    ...
    // When any mesh entity is modified, topology is changed...
    ...
    JNodeSequence  getNodes(const std::string &attribname) const;
    ...
    void deleteNodeAttribute(const std::string &s);
    ...
    JEdgeSequence getEdges(const std::string &s);
    ...
    void  deleteEdgeAttribute(const std::string &s);
    ...
    JFaceSequence getFaces( const std::string &s) const;
    ...
    void deleteFaceAttribute(const std::string &s);
    ...
    JCellSequence getCells(const std::string &s) const;
    ...
    void deleteCells();
    void deleteCellAttribute(const std::string &s);
...
private:
    ...
    JMeshGeometryPtr  geometry;
    JMeshTopologyPtr  topology;

    std::unique_ptr<JAttributeManager>  attribManager;
    std::unique_ptr<JMeshRelationManager>  relManager;
...
struct JMeshIO {
    static JMeshPtr readFile( const std::string &f);
    static int  saveAs( const JMeshPtr &m, const std::string &f);
};
