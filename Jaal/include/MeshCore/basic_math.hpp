...
struct LinearSystem {
    static int solve(Eigen::SparseMatrix<double> &K, std::vector<double> &b, std::vector<double> &x);
};
...
template<class T>
inline double dot_product( const std::array<T,2> &A, const std::array<T,2> &B)
{
...
template<class T>
inline double getVecAngle( const std::array<T,3> &A, const std::array<T,3> &B, int measure)
{
