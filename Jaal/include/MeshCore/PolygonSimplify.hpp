...
class JPolygonSimplify
{
public:
    static const int DOUGLAS_PEUCKER  = 0;
    static const int DOUGLAS_PEUCKERN = 1;
    static const int NTH_POINT = 2;
    static const int LANG = 3;
    static const int OPHEIM  = 4;
    static const int PERPENDICULAR_DISTANCE = 5;
    static const int RADIAL_DISTANCE = 6;
    static const int REUMANN_WITKAM  = 7;

    static int getAlgorithmID( const string &s);

    void setMesh( const JMeshPtr &m);
    void setSkipParity( int p )
    {
        skipParity = p;
    }

    JMeshPtr    getSimplified( int algo, double tol);
private:
    JMeshPtr   mesh;
    JMeshPtr   simplified;
    int        algo = DOUGLAS_PEUCKER;
    double     tol  = 0.00001;
    void       getSimplified( const JEdgeSequence &s);
    int        count; // only for douglas_peucker_n algorithm,
    int        skipParity = 0;
};
