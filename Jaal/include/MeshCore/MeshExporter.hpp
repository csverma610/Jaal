...
struct JMeshOFFExporter : public JMeshExporter {
    ~JMeshOFFExporter() {}
    int writeFile(const JMeshPtr &m, const std::string & s);
};
...
struct JMeshPOVExporter : public JMeshExporter {
    ...
    int writeFile(const JMeshPtr &m, const std::string & s);
private:
    void writeheader(const JMeshPtr &m, std::ofstream & s);
    void writemesh(const JMeshPtr &m,   std::ofstream & s);
};
...
private:
    ...
    template<class T>
    int writeNodeAttribute(const std::string &name, const T &dummy, std::ofstream &ofile)
    {
    ...
    template<class T>
    int writeEdgeAttribute(const std::string &name, const T &dummy, std::ofstream &ofile)
    {
        T val;
        size_t numActive= 0, nCount = 0;
