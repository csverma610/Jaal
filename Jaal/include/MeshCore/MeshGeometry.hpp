...
public:
    ...
    static JHexahedronPtr getMinimumBox( const vector<Point3D> &pnts);
    static JHexahedronPtr getMinimumBox( const JNodeSequence &nodes);
    static vector<double> getEuclideanDistance( const JMeshPtr &a, const JMeshPtr &b);

    static double getMaximumDistance( const JMeshPtr &m1, const JMeshPtr &m2);
    ...
    double  getMeanBoundaryEdgeLength() const;
