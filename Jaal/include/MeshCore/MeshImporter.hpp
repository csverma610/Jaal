...
typedef std::shared_ptr<JMeshImporter> JMeshImporterPtr;
...
private:
    vector<double> nodeCoords, texCoords;
    vector<size_t> triConnect, quadConnect;
    vector<vector<size_t>> polyConnect;
    vector<size_t> texTriConnect, texQuadConnect, texPolyConnect;

    bool has_texture;
    void readFaceLine(const string &s, vector<size_t> &v);
    JMeshPtr getMesh();
