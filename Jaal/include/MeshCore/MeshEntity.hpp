...
template <typename T>
inline std::string get_pod_name( T )
{
    return "Unknown";
}

template<>
inline std::string get_pod_name( int )
{
    return "int";
}

template<>
inline std::string get_pod_name( short int)
{
    return "sint";
}

template<>
inline std::string get_pod_name( float )
{
    return "float";
}

template<>
inline std::string get_pod_name( double )
{
    return "double";
}

template<>
inline std::string get_pod_name( char  )
{
    return "char";
}

template<>
inline std::string get_pod_name( unsigned char )
{
    return "uchar";
}

template<>
inline std::string get_pod_name( const std::string )
{
    return "string";
}
...
using  JMeshEntityPtr = std::shared_ptr<JMeshEntity>;
using  JNodePtr = std::shared_ptr<JNode>;
using  JEdgePtr = std::shared_ptr<JEdge>;
using  JFacePtr = std::shared_ptr<JFace>;
using  JCellPtr = std::shared_ptr<JCell>;
using  JMeshPtr = std::shared_ptr<JMesh>;

using  JNodeSequence = std::vector<JNodePtr>;
using  JEdgeSequence = std::vector<JEdgePtr>;
using  JFaceSequence = std::vector<JFacePtr>;
using  JCellSequence = std::vector<JCellPtr>;

using  JNodeList = std::list<JNodePtr>;
using  JEdgeList = std::list<JEdgePtr>;
using  JFaceList = std::list<JFacePtr>;
using  JCellList = std::list<JCellPtr>;

using  JNodeSet = std::set<JNodePtr>;
using  JEdgeSet = std::set<JEdgePtr>;
using  JFaceSet = std::set<JFacePtr>;
using  JCellSet = std::set<JCellPtr>;
...
using  JTrianglePtr      = std::shared_ptr<JTriangle>;
using  JPolygonPtr       = std::shared_ptr<JPolygon>;
using  JQuadrilateralPtr = std::shared_ptr<JQuadrilateral>;
using  JTetrahedronPtr   = std::shared_ptr<JTetrahedron>;
using  JHexahedronPtr    = std::shared_ptr<JHexahedron>;
using  JPolyhedronPtr    = std::shared_ptr<JPolyhedron>;
using  JTriangularPrismPtr = std::shared_ptr<JTriangularPrism>;

using  JFacePair = std::pair<JFacePtr, JFacePtr>;
...
public:
    ...
    static bool isChar(const std::any & operand);
    static bool isFloat(const std::any & operand);
    static bool isDouble(const std::any & operand);
    static bool isInteger(const std::any & operand);
    static bool isString(const std::any & operand);

    JAttribute() {}

    JAttribute(const std::string &s, const std::any & p)
    {
        name = s;
        value = p;
    }
...
inline bool JAttribute :: isChar( const std::any &operand)
{
    return operand.type() == typeid(char);
}

inline bool JAttribute :: isFloat( const std::any &operand)
{
    return operand.type() == typeid(float);
}

inline bool JAttribute :: isDouble( const std::any &operand)
{
    return operand.type() == typeid(double);
}

inline bool JAttribute :: isInteger( const std::any &operand)
{
    return operand.type() == typeid(int);
}

inline bool JAttribute :: isString( const std::any &operand)
{
...
typedef std::shared_ptr<JRelationManager> JRelationManagerPtr;
...
{
public:
    typedef std::shared_ptr<JAttribute> JAttributePtr;
    ...
    template<class T>
    int setAttribute(const std::string &s, const T &val)
    {
    ...
    int getAttribute(const std::string &s, std::any &attrib)
    {
        int nAttribs = attributes.size();
        for( int i = 0; i < nAttribs; i++) {
    ...
    template<class T>
    int getAttribute(const std::string &s, T &val) const
    {
        int nAttribs = attributes.size();
        for( int i = 0; i < nAttribs; i++) {
            if( attributes[i]->name == s ) {
                val = std::any_cast<T>(attributes[i]->value);
                return 0;
            }
        }
        return 1;
    ...
    void deleteAttribute( const std::string &s)
    {
...
typedef std::shared_ptr<JAttributeManager> JAttributeManagerPtr;
...
public:
    ...
    std::vector<std::string> getAttributeNames() const
    {
        return attribManager->getAttributeNames();
    }

    int hasAttribute(const std::string &s) const
    {
...
protected:
    ...
    std::unique_ptr<JRelationManager>  relationManager;
    std::unique_ptr<JAttributeManager> attribManager;
};

///////////////////////////////////////////////////////////////////////////////

struct JEntityRemovedPred {
...
// contain with the vertex..
    ...
    std::unique_ptr<JRelationManager> hashRel;
};
...
{
    JNodePtr vtx = std::dynamic_pointer_cast<JNode>(entity);
    assert(vtx);
    return vtx;
}
...
public:
    ...
    JNodePtr getHashNode() const
    {
        if( !nodes.empty() ) return *min_element(nodes.begin(), nodes.end());
        JNodePtr nullPtr;
        return nullPtr;
    }
...
//  JFace() { }
    ...
    int getEdges( JEdgeSequence &edges) const
    {
        int  nn = getSize(0);
        if( nn < 3) return 1;
        edges.resize(nn);
...
//  static void getCentroid( const double *x, const double *y, int n, double *c);
    ...
    static int    concaveAt( const JFacePtr &f);
...
{
    static std::map<std::string,std::string> attribInfo;
public:
    static size_t AnyObjectModified;
    ...
    virtual int getEdges( JEdgeSequence &e) const = 0;
    virtual int getFaces( JFaceSequence &f) const = 0;
...
{
    JCellPtr cell = std::dynamic_pointer_cast<JCell>(entity);
    return cell;
}
...
public:
    ...
    int getHexCells( std::vector<JHexahedron> &h) const;
...
public:
    ...
    int getEdges( JEdgeSequence &eseq) const
    {
...
{
    JTrianglePtr tri;
    tri = std::dynamic_pointer_cast<JTriangle>(f);
    return tri;
}
...
JQuadrilateralPtr JQuadrilateral::down_cast( const JFacePtr &f)
{
    ...
    quad = std::dynamic_pointer_cast<JQuadrilateral>(f);
    return quad;
}
...
{
    JTetrahedronPtr tet = std::dynamic_pointer_cast<JTetrahedron>(c);
    return tet;
}
...
{
    JHexahedronPtr hex = std::dynamic_pointer_cast<JHexahedron>(c);
    return hex;
}
...
void JRelationManager::addRelation(const JNodePtr &vertex, int type)
{
    ...
    if( type == SORTED_RELATIONS ) rstl::sort(relations0);
}
///////////////////////////////////////////////////////////////////////////////
...
void JRelationManager::addRelation(const JEdgePtr &edge, int type)
{
    ...
    if( type == SORTED_RELATIONS ) rstl::sort( relations1 );
}
///////////////////////////////////////////////////////////////////////////////
...
void JRelationManager ::addRelation(const JFacePtr &face, int type)
{
    ...
    if( type == SORTED_RELATIONS ) rstl::sort(relations2);
}
///////////////////////////////////////////////////////////////////////////////
...
void JRelationManager ::addRelation(const JCellPtr &cell, int type)
{
    ...
    if( type == SORTED_RELATIONS ) rstl::sort(relations3);
}
///////////////////////////////////////////////////////////////////////////////
...
void JRelationManager::removeRelation(const JNodePtr &vertex)
{
    if (hasRelation(vertex))
        rstl::remove_erase(relations0, vertex);
}
///////////////////////////////////////////////////////////////////////////////
...
{
    if (hasRelation(edge))
        rstl::remove_erase(relations1, edge);
}
///////////////////////////////////////////////////////////////////////////////
...
{
    if (hasRelation(face))
        rstl::remove_erase(relations2, face);
}
///////////////////////////////////////////////////////////////////////////////
...
{
    if (hasRelation(cell))
        rstl::remove_erase(relations3, cell);
}
...
bool JRelationManager ::hasRelation(const JNodePtr &vertex) const
{
    ...
    return rstl::contains( relations0, vertex);
}
...
inline bool JRelationManager ::hasRelation(const JEdgePtr &edge) const
{
    ...
    return rstl::contains( relations1, edge);
}
///////////////////////////////////////////////////////////////////////////////
...
inline bool JRelationManager ::hasRelation(const JFacePtr &face) const
{
    ...
    return rstl::contains(relations2, face);
}
...
inline bool JRelationManager ::hasRelation(const JCellPtr &cell) const
{
    ...
    return rstl::contains(relations3, cell);

    /*
    auto it = std::find(relations3, cell);
    if( it == std::end(relations3)) return 0;
    */
...
inline void JRelationManager :: prune( int e )
{
    if( e == 0) {
        ...
        vend = remove_if(relations0.begin(), relations0.end(), JEntityRemovedPred());
    ...
    if( e == 1) {
        ...
        eend = remove_if(relations1.begin(), relations1.end(), JEntityRemovedPred());
    ...
    if( e == 2) {
        ...
        fend = remove_if(relations2.begin(), relations2.end(), JEntityRemovedPred());
    ...
    if( e == 3) {
        ...
        cend = remove_if(relations3.begin(), relations3.end(), JEntityRemovedPred());
