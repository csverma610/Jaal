...
typedef std::shared_ptr<JDualSheet>  JDualSheetPtr;
typedef std::shared_ptr<JDualChord>  JDualChordPtr;
typedef std::shared_ptr<JQuadDual>   JQuadDualPtr;
typedef std::shared_ptr<JHexDual>    JHexDualPtr;

struct JDualSheet {

    static JDualSheetPtr joinSheets( const JDualSheetPtr &a, const JDualSheetPtr &b);
...
public:
    static const int BOUNDARY_CHORD  = 0;
    static const int CYCLIC_CHORD    = 1;

    static const int SIMPLE_CHORD    = 0;
    static const int SELF_TOUCHING_CHORD      = 1;
    static const int SELF_INTERSECTING_CHORD  = 2;
    static const int SELF_INTERSECTING_AND_TOUCHING_CHORD  = 3;
...
public:
    ...
    {
        if( start_at_boundary || end_at_boundary ) return 1;
        return 0;
    }

#ifdef CSV
    virtual JEdgeSequence getSegments() const {}
    virtual JFaceSequence getMeshFaces() const {}
    virtual void  deleteSegments() const {}
    virtual int   flip() {}
    ...
    void append(const JDualChord *other);

    void addIntersection( const JFacePtr &face)
    {
        selfIntersectingFaces.push_back(face);
        self_intersecting = 1;
    }

    void addIntersection( const JCellPtr &cell)
    {
        selfIntersectingCells.push_back(cell);
        self_intersecting = 1;
    }

    void addElements( const JEdgePtr &edge, const JFacePtr &face = nullptr);
    void addElements( const JFacePtr &face, const JEdgePtr &edge, const JCellPtr &cell = nullptr);

    void get_parallel_nodes(JNodeSequence &seq) const;

    void getNodes( JNodeSequence &seq) const;
    void getEdges( JEdgeSequence &seq) const;
    void getFaces( JFaceSequence &seq) const;
    void getCells( JCellSequence &seq) const;

    void  getSegments(JEdgeSequence &eseq) const
    {
        eseq = chordSegments;
    }

    void getIntersectingFaces( JFaceSequence &seq) const
    {
        seq = selfIntersectingFaces;
    }
    ...
    bool isSimple() const
    {
        if( classify() == SIMPLE_CHORD) return 1;
        return 0;
    }
    ...
    int classify() const
    {
        if( self_touching && self_intersecting ) return SELF_INTERSECTING_AND_TOUCHING_CHORD;
        if( self_touching ) return SELF_TOUCHING_CHORD;
        if( self_intersecting ) return SELF_INTERSECTING_CHORD;
        return SIMPLE_CHORD;
    }
private:
    struct Element {
...
class JHexChord;
typedef std::shared_ptr<JHexChord> JHexChordPtr;
...
/*
inline
bool DualChord :: hasEdge( const Edge *e) const
{
     size_t nSize = dElements.size();
     for( size_t i = 0; i < nSize; i++) {
	  if( dElements[i].parEdge1 == e ) return 1;
	  if( dElements[i].parEdge2 == e ) return 1;
     }
     return 0;
}

inline
bool DualChord :: hasFace( const Face *f) const
{
     size_t nSize = dElements.size();
     for( size_t i = 0; i < nSize; i++)
	  if( dElements[i].face == f ) return 1;
     return 0;
}

inline
bool DualChord :: hasCell( const Cell *c) const
{
     size_t nSize = dElements.size();
     for( size_t i = 0; i < nSize; i++)
	  if( dElements[i].cell == c ) return 1;
