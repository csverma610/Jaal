...
public:
    ...
    JMeshPtr mesh;
    std::tuple<int,int> getRange(int dir) const;
...
private:
    ...
    std::tuple<int,int>  getXRange() const;
    std::tuple<int,int>  getYRange() const;
    std::tuple<int,int>  getZRange() const;
    bool isBoundary(int i, int j, int k) const;
};

typedef std::shared_ptr<JVoxelMesh>  JVoxelMeshPtr;
